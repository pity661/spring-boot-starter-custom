...
public class ExecutorExample {
  ...
  private static void executorBlockingTest() {
    ...
    ThreadPoolExecutor executorService =
        new ThreadPoolExecutor(
            ...
            linkedBlockingQueue,
            threadFactory,
            (a, b) -> System.out.println(String.format("workId被抛弃了:[%d]", getWork(a).getWorkId())));
    ...
    linkedBlockingQueue
        ...
        .forEachRemaining(
            runnable ->
                System.out.println(String.format("队列中workId:[%d]", getWork(runnable).getWorkId())));
    ...
    try {
      ...
      Field taskField =
          Stream.of(Executors.class.getDeclaredClasses())
              .filter(
                  single ->
                      "java.util.concurrent.Executors$RunnableAdapter".equals(single.getName()))
              .findFirst()
              .get()
  ...
  public static class Work implements Runnable {
    ...
    public void run() {
      System.out.println(
          String.format("threadName:[%s], workId:[%d]", Thread.currentThread().getName(), workId));
      try {
